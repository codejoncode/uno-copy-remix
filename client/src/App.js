...
class App extends Component {
  ...
  selectingCard = index => async () => {
    ...
    await this.state.game.mainGamePlay(this.state.game.currentPlayer, index);
    const currentUsersHandAttempt = this.state.game.currentPlayer.player
      .gatherForPlay;
    this.setState({
      ...
      <div>
        {gameStarted && (
          <SideBar
            ...
            activePlayer={currentPlayer.player.name}
          />
        )}
        ...
        {gameStarted && currentPlayer && (
          <div>
            {gameStarted && currentPlayer && (
              <div>
                <button onClick={this.makePlay()}>Play</button>{" "}
                <button onClick={this.pickUp(currentPlayer, "pickup")}>
                  Pick Up
                </button>{" "}
                <button onClick={this.nextPlayer()}>next player</button>
              </div>
            )}
            {currentPlayer && currentPlayer.player.hand < 2 && (
            ...
            {currentPlayer &&
              currentPlayer.player.hand.map((card, index) => (
                <div
                  className="clickableCard"
                  onClick={this.selectingCard(index)}
                  key={index}
                >
                  <Card card={card} />
                </div>
        ...
        <br />
        {currentUsersHandAttempt &&
          currentUsersHandAttempt.map((card, index) => (
            <div
              className="clickableCard"
              key={index}
              onClick={this.clearCard(index)}
            >
